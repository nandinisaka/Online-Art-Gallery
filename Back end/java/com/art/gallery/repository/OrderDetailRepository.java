...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Long> {
	...
	@Modifying
	@Query(value = "insert into order_detail (order_id, art_id, quantity, mrp_price, price, amount, payment_id, order_status) values (:order, :art, :quantity, :mrpPrice, :amount, :totalPrice, :paymentId, :orderStatus, :paymentMode)", nativeQuery = true)
	void saveOrderDetails(@Param("order") Order orders, @Param("art") Art art, @Param("quantity") int quantity,
			@Param("mrpPrice") double mrpPrice, @Param("amount") double amount, @Param("totalPrice") double totalPrice, @Param("paymentId") double paymentId, @Param("orderStatus") String orderStatus);
