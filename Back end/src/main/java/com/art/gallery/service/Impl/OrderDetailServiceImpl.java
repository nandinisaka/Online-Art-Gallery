...
import com.art.galley.repository.OrderDetailRepository;
...
public class OrderDetailServiceImpl implements OrderDetailService {
	...
	@Override
	public boolean saveCartOrderDetail(Order orders, Art arts, int quantity, double mrpPrice, double amount, double totalPrice,
			int paymentId, String orderStatus, String paymentMode) {
		boolean status = false;
		if (orders != null) {
			status = true;
			orderDetailRepo.saveOrderDetails(orders, arts, quantity, mrpPrice, amount, totalPrice, paymentId, orderStatus, paymentMode);
		}
		return status;
	}
	...
	public void sendPdfEmail(String name, String email, int OrderNum, byte[] bytes) {
		...
		try {
			...
			helper.setSubject("Nandini Art Gallery: Order#" + OrderNum);
